...
import { useProfileMutation } from "../../redux/api/useraApiSlice";
...
const Profile = () => {
  ...
  const [updateProfile, { isLoading:loadingUadateProfile }] = useProfileMutation();
  ...
  const submitHandler = async (e) => {
    e.preventDefault()
      try {
        const res = await updateProfile({ _id: userInfo.data.user._id, username, email }).unwrap()
    ...
    <div className="container mx-auto p-4 mt-[2rem]">
      <div className="flex justify-center align-center md:flex md:space-x-4">
        ...
        <form onSubmit={submitHandler}  className="container w-[40rem]">
            ...
            <div className="my-[2rem] ">
                <lable htmlFor="username" className="text-black block text-sm font-medium ">
                    Username
                </lable>
                <input type="text" id="username" className="mt-1 p-2 border rounded w-full" value={username}  onChange={(e)=>setUsername(e.target.value)}/>
            </div>
            ...
            <div className="flex justify-between">
              ...
              <Link to={"/user-orders"}  className="bg-pink-500 text-black py-2 px-4 rounded hover:bg-pink-600">
                My Orders
              </Link>
            </div>
                </form>
